...
export default function Cities() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .get(
        `https://api.openweathermap.org/data/2.5/forecast?q=${cityName}&appid=2e9ac1f3ad86ab8897cfda73dfecad7b`
      )
      .then((response) => {
    ...
    <div className="flex md:flex-row flex-col md:h-screen bg-black">
      ...
      <div className="md:basis-7/12 mx-5 basis-10/12 flex flex-col text-white">
        <form onSubmit={handleSubmit}>
          <input
            className=" bg-gray-800 w-full mt-6 p-5 rounded-3xl focus:outline-none relative"
            type="text"
        ...
        <div>
          <div className="mr-4 w-full">
            {cityList.map((city, index) => (
              <div
                key={index}
                className="bg-gray-800 text-white flex justify-between items-center p-5 mt-5 rounded-3xl h-3/6"
              >
                <div>
                  <p>{cityList[index]}</p>
                </div>
                <div className="flex gap-5">
                  <button
                    className="bg-blue-600 w-24 h-10 rounded-2xl hover:bg-blue-800"
                    type="button"
                  ...
                  <button
                    className="bg-red-500 w-24 h-10 rounded-2xl hover:bg-red-800"
                    type="button"
            ...
            <div className="md:mt-14 mb-32 h-auto p-6 bg-gray-800 text-white rounded-3xl">
              <div className="flex flex-col h-full">
                <div className="basis-5/12 p-5 pt-12 relative">
                  <img
                    className="w-44 h-44 absolute top-1/5 right-3"
                    src={getImageUrl(
                      value[openIndex].data.list[0].weather[0].icon
                    )}
                  />
                  <h1 className="flex-1 font-bold text-5xl mb-1">
                    {cityList[openIndex]}
                  </h1>
                  <span>
                    {value[openIndex].data.list[0].weather[0].description}
                  </span>
                  <h1 className="flex-1 font-bold text-6xl mt-10">
                    {parseInt(value[openIndex].data.list[0].main.temp - 273)}°C
                  </h1>
                </div>
              </div>
              <div>
                <h1 className="font-bold text-xl">3 Days Forcast</h1>
                <br></br>
                    ...
                    <p>
                      {
                        weekday[
                          new Date(
                            value[openIndex].data.list[0].dt_txt
                          ).getDay()
                        ]
                      }
                    </p>
                ...
                <div className="flex justify-between items-center my-4 px-2">
                  <div className="flex w-3/4 justify-between">
                    <p>
                      {
                        weekday[
                          new Date(
                            value[openIndex].data.list[7].dt_txt
                          ).getDay()
                        ]
                      }
                    </p>
                    <div className="flex w-2/4 justify-between">
                      <img
                        className="w-8 h-8"
                        src={getImageUrl(
                          value[openIndex].data.list[7].weather[0].icon
                        )}
                      />
                  ...
                  <p>
                    {parseInt(value[openIndex].data.list[7].main.temp - 273)}°C
                  </p>
                </div>
                <hr className="border-solid border-gray-600"></hr>
                <br></br>
                <div className="flex justify-between items-center my-4 px-2">
                  <div className="flex w-3/4 justify-between">
                    <p>
                      {
                        weekday[
                          new Date(
                            value[openIndex].data.list[15].dt_txt
                          ).getDay()
                        ]
                      }
                    </p>
                    <div className="flex w-2/4 justify-between">
                      <img
                        className="w-8 h-8"
                        src={getImageUrl(
                          value[openIndex].data.list[15].weather[0].icon
                        )}
                      />
                  ...
                  <p>
                    {parseInt(value[openIndex].data.list[15].main.temp - 273)}°C
                  </p>
                </div>
                <hr className="border-solid border-gray-600"></hr>
                <br></br>
